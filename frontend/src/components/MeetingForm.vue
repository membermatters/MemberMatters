<template>
  <q-card :flat="!!meetingId">
    <q-card-section>
      <div class="text-h6">
        {{ meetingId ? $t('meetingForm.editMeeting') : $t('meetingForm.newMeeting') }}
      </div>
    </q-card-section>

    <q-card-section>
      <p>
        {{ meetingId ? $t('meetingForm.editDescription') : $t('meetingForm.pageDescription') }}
      </p>
      ...
      >
        ...
        <div>
          ...
          <q-input
            ...
            :rules="[
              ...
              val => validateFutureDateTime(val, pastMeeting) || $t('validation.futureDate'),
            ]"
            class="q-mb-sm"
        ...
        >
          {{ !!meetingId ? $t('meetingForm.editSuccess') : $t('meetingForm.success') }}
        </q-banner>
...
export default {
  ...
  methods: {
    ...
    updateMeeting() {
      ...
      this.$axios.put(`/api/meetings/${this.meetingId}/`, this.form)
        .then(() => {
  ...
  mounted() {
    ...
    if (this.meetingId) {
      const meetingInfo = this.meetings[this.meetings.findIndex((p) => p.id === this.meetingId)];
