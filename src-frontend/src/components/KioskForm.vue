  ...
  <q-card flat>
    ...
    <q-card-section>
      <q-form @submit="updateKiosk()">
        ...
        <q-checkbox v-model="form.authorised" :label="$t('kiosk.authorised')" />

        <q-checkbox v-model="form.playTheme" :label="$t('form.playTheme')" />
...
export default {
  ...
  methods: {
    ...
    updateKiosk() {
      ...
      this.$axios
        .put(`/api/kiosks/${this.kioskId}/`, this.form)
        .then(() => {
  ...
  mounted() {
    const kioskInfo =
      this.kiosks[this.kiosks.findIndex((p) => p.id === this.kioskId)];
